...
Vendored copy of ``Sigima/scripts/run_with_env.py`` so that DataLab-Web has
the same VS Code task ergonomics as its sibling Python projects.

It auto-detects the best Python interpreter to use:

1. ``PYTHON`` variable in ``.env`` (e.g. for WinPython distributions)
