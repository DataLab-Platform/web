...
Macros and notebooks you edit are kept in a roll-over IndexedDB cache
so you can re-open them later, but they are **not** silently restored
into a fresh session: they only appear in each panel's **Recent…**
menu. Pristine, auto-created sample documents are never cached.
...
- The window title shows `DataLab-Web — <filename or "Untitled">`,
  ...
  changes, cleared on the next **Open / Save HDF5 workspace…**.
- A `beforeunload` confirmation prompt fires only when the workspace
  is dirty.
- A one-time informational banner appears at cold start if the
  IndexedDB cache holds edited macros or notebooks, pointing you at
  the **Recent…** menus where you can re-open them. **Dismiss** hides
  it for the session.
