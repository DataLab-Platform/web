...
def dataset_to_schema_with_values(instance: gdt.DataSet) -> dict[str, Any]:
    ...
    # very first render already greys out inactive widgets (e.g. the
    # blob-detection ``min_circularity`` gated by ``filter_by_circularity``)
    # without waiting for a frontend round-trip.  Only inactive items are
    # flagged; active items stay editable by default, and dynamic ones keep
    # their ``x-guidata-active-dynamic`` marker so the frontend re-evaluates
    # them via :func:`resolve_dataset_active` on every edit.
...
def _sync_store_props(instance: gdt.DataSet) -> None:
    ...
    mechanism (used e.g. by ``BlobOpenCVParam.filter_by_circularity`` to
    gate ``min_circularity`` / ``max_circularity``) holds its value on the
...
# ---------------------------------------------------------------------------
# Shim wiring for DataLab-Web
# ---------------------------------------------------------------------------
# Until the released ``guidata`` ships ``dataset_to_schema`` etc., expose
